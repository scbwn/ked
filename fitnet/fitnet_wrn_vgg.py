...
def model_nn_soft(d, num_of_classes):
    ...
    shared_op = shared_layers(inp)
    branch_op=branch_layers(shared_op)
    proj_op = UpSampling2D((8,8))(resnet_layer(branch_op, num_filters=proj_dim, kernel_size=3))
    flat_inp=Flatten()(branch_op)
    x=Dense(512, activation='relu', kernel_initializer='he_normal', 
                       kernel_regularizer=regularizers.l2(decay_const))(flat_inp)
    x=Dense(512, activation='relu', kernel_initializer='he_normal', 
    ...
    nn.compile(SGD(momentum=0.9, nesterov=True), loss=['categorical_crossentropy', 'kl_divergence', 'mse'], 
...
def model_nn_ked(d, num_of_classes):
    ...
    for i in range(M):
        ...
        proj_op_list.append(UpSampling2D((8,8))(resnet_layer(branch_op, num_filters=new_proj_dim[i], kernel_size=3)))
        flat_inp=Flatten()(branch_op)
