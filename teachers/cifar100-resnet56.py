...
def shared_layers(x, filters=16*2, depth=56):
    if (depth - 2) % 6 != 0:
...
def branch_layers(x, filters=64*2, depth=56):
    if (depth - 2) % 6 != 0:
        raise ValueError('depth should be 6n+2 (eg 20, 32, 44 in [a])')
    num_filters = filters
    num_res_blocks = int((depth - 2) / 6)
...
teacher.save('./models/cifar100-resnet56/teacher.h5')
...
print(confusion_matrix(y_test, np.argmax(y_pred_test,1)))
print(bootstrap_score(y_test, np.argmax(y_pred_test,1)))
new_teacher.save('./models/cifar100-resnet56/new_teacher.h5')