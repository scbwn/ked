...
def shared_layers(x, filters=16*4, depth=28):
    if (depth - 4) % 6 != 0:
...
def branch_layers(x, filters=64*4, depth=28):
    if (depth - 4) % 6 != 0:
        raise ValueError('depth should be 6n+4 (eg 16, 28, 40 in [a])')
    num_filters = filters
    num_res_blocks = int((depth - 4)/6)
...
def model_exp_teacher(d, num_of_classes):
    ...
    inp=Input(shape=d)
    shared_op = shared_layers(inp)
    shared_op = resnet_layer(shared_op, num_filters=M*122, kernel_size=1)
    shared_op = Layer(name='shared_op')(shared_op)
    shared_op_list=tf.split(shared_op, M, axis=-1)
    logits=[]
    for i in range(M):
        flat_inp=branch_layers(shared_op_list[i], filters=122)
...
teacher.save('./models/cifar100-wrn-28-4/teacher.h5')
...
print(confusion_matrix(y_test, np.argmax(y_pred_test,1)))
print(bootstrap_score(y_test, np.argmax(y_pred_test,1)))
new_teacher.save('./models/cifar100-wrn-28-4/new_teacher.h5')