...
def model_nn_student(d, num_of_classes):
    ...
    flat_inp=Flatten()(branch_op)
    x=Dense(512, activation='relu', kernel_initializer='he_normal', 
                       kernel_regularizer=regularizers.l2(decay_const))(flat_inp)
    x=Dense(512, activation='relu', kernel_initializer='he_normal', 
                       kernel_regularizer=regularizers.l2(decay_const))(x)
...
def model_nn_soft(d, num_of_classes):
    ...
    shared_op = shared_layers(inp)
    branch_op=branch_layers(shared_op)
    proj_op = UpSampling2D((8,8))(branch_op)
    flat_inp=Flatten()(branch_op)
    x=Dense(512, activation='relu', kernel_initializer='he_normal', 
                       kernel_regularizer=regularizers.l2(decay_const))(flat_inp)
    x=Dense(512, activation='relu', kernel_initializer='he_normal', 
...
def model_nn_ked(d, num_of_classes):
    ...
    for i in range(M):
        ...
        proj_op_list.append(UpSampling2D((8,8))(branch_op))
        flat_inp=Flatten()(branch_op)
        x=Dense(filters_per_branch, activation='relu', kernel_initializer='he_normal', 
                           kernel_regularizer=regularizers.l2(decay_const))(flat_inp)
        x=Dense(filters_per_branch, activation='relu', kernel_initializer='he_normal', 
                           kernel_regularizer=regularizers.l2(decay_const))(x)
...
def custom_loss(y_true, y_pred):
    q_true=Flatten()(tf.norm(y_true, axis=3)**2)
    q_pred=Flatten()(tf.norm(y_pred, axis=3)**2)
    return 1000*mean_squared_error(q_true/tf.norm(q_true, axis=1, keepdims=True),
                            q_pred/tf.norm(q_pred, axis=1, keepdims=True))
